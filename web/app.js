...
function renderLayerButtons(){
  ...
  const order=['complete','periods','enso','terrain'];
...
function startDrawing(kind){
  ...
  const options={shapeOptions:{color:'#f3b63f',weight:2,fillColor:'#f3b63f',fillOpacity:.12}};const drawer=kind==='rectangle'?new L.Draw.Rectangle(map,options):new L.Draw.Polygon(map,{...options,allowIntersection:false,showArea:true});drawer.enable()
}
...
document.getElementById('draw-rectangle').addEventListener('click',()=>startDrawing('rectangle'));document.getElementById('draw-polygon').addEventListener('click',()=>startDrawing('polygon'));
